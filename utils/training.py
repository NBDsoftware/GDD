...
def loss_function(
    ...
    tr_sigma, rot_sigma, tor_sigma = t_to_sigma(
        *[
            (
                torch.cat([d.complex_t[noise_type] for d in data])
                if device.type == "cuda"
                else data.complex_t[noise_type]
            )
    ...
    rot_loss = (((rot_pred.cpu() - rot_score) / rot_score_norm) ** 2).mean(
        dim=mean_dims
    )
    ...
    if not no_torsion:
        edge_tor_sigma = torch.from_numpy(
            np.concatenate(
                [d.tor_sigma_edge for d in data]
                if device.type == "cuda"
                else data.tor_sigma_edge
            )
        )
        tor_score = (
        ...
        tor_score_norm2 = torch.tensor(
            torus.score_norm(edge_tor_sigma.cpu().numpy())
        ).float()
        ...
        if apply_mean:
            tor_loss, tor_base_loss = torch.zeros(1, dtype=torch.float), torch.zeros(
                1, dtype=torch.float
            )
        else:
            tor_loss, tor_base_loss = torch.zeros(
                len(rot_loss), dtype=torch.float
            ), torch.zeros(len(rot_loss), dtype=torch.float)
