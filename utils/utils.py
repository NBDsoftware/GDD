...
def get_model(args, device, t_to_sigma, no_parallel=False, confidence_mode=False):
    ...
    model = model_class(
        ...
        num_confidence_outputs=(
            ...
            if "rmsd_classification_cutoff" in args
...
def convert_string_to_float(item):
    """Convert strings in mixed data types to floats"""
    if isinstance(item, str):  # Check if the item is a string
        try:
            return float(item)  # Convert string to float
        except ValueError:
            return item  # Return the item unchanged if conversion fails
    elif isinstance(item, list):  # Check if the item is a list
        return [
            convert_string_to_float(sub_item) for sub_item in item
