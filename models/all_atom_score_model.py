...
class TensorProductScoreModel(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        # build ligand graph
        lig_node_attr, lig_edge_index, lig_edge_attr, lig_edge_sh = (
        ...
        rec_node_attr, rec_edge_index, rec_edge_attr, rec_edge_sh = (
        ...
        # build atom graph
        atom_node_attr, atom_edge_index, atom_edge_attr, atom_edge_sh = (
        ...
        for l in range(self.num_conv_layers):
            ...
            lig_node_attr = F.pad(
                lig_node_attr, (0, lig_update.shape[-1] - lig_node_attr.shape[-1])
            )
            lig_node_attr = lig_node_attr + lig_update + la_update + lr_update

            if l != self.num_conv_layers - 1:  # last layer optimisation
                atom_node_attr = F.pad(
                    atom_node_attr, (0, atom_update.shape[-1] - rec_node_attr.shape[-1])
                )
                atom_node_attr = atom_node_attr + atom_update + al_update + ar_update
                rec_node_attr = F.pad(
                    rec_node_attr, (0, rec_update.shape[-1] - rec_node_attr.shape[-1])
                )
        ...
        center_edge_index, center_edge_attr, center_edge_sh = (
        ...
        edge_sigma = tor_sigma[data["ligand"].batch][
            data["ligand", "ligand"].edge_index[0]
