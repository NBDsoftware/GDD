...
def get_cache_path(args, split):
    ...
    cache_path = os.path.join(
        ...
        + (
            ...
            else f"_atomRad{args.atom_radius}_atomMax{args.atom_max_neighbors}"
        )
        + (
            ""
            if args.no_torsion or args.num_conformers == 1
            else f"_confs{args.num_conformers}"
        )
...
class ConfidenceDataset(Dataset):
    def __init__(
        ...
        if (
            not os.path.exists(
                os.path.join(self.full_cache_path, "ligand_positions.pkl")
            )
        ...
        print(
            ...
            if self.use_original_model_cache
            else "Not using the cached complex graphs of the original model args. Instead the complex graphs are used that are at the location given by the dataset parameters given to confidence_train.py"
        )
        ...
        if self.cache_ids_to_combine is None:
            ...
            with open(
                os.path.join(self.full_cache_path, "ligand_positions.pkl"), "rb"
            ) as f:
                self.full_ligand_positions, self.rmsds = pickle.load(f)
            if os.path.exists(
                os.path.join(self.full_cache_path, "complex_names_in_same_order.pkl")
            ):
                with open(
                    os.path.join(
                        self.full_cache_path, "complex_names_in_same_order.pkl"
                    ),
                    "rb",
                ) as f:
                ...
                with open(
                    os.path.join(original_model_cache, "heterographs.pkl"), "rb"
                ) as f:
                    original_model_complex_graphs = pickle.load(f)
                    generated_rmsd_complex_names = [
                        d.name for d in original_model_complex_graphs
                    ]
            ...
            for idx, cache_id in enumerate(self.cache_ids_to_combine):
                ...
                if not os.path.exists(
                    os.path.join(
                        self.full_cache_path, f"ligand_positions_id{cache_id}.pkl"
                    )
                ):
                    raise Exception(
                        f"The generated ligand positions with cache_id do not exist: {cache_id}"
                    )  # be careful with changing this error message since it is sometimes cought in a try catch
                with open(
                    os.path.join(
                        self.full_cache_path, f"ligand_positions_id{cache_id}.pkl"
                    ),
            ...
            for idx, (
                ...
                name_to_pos_dict = {
                    name: (rmsd, pos)
                    for name, rmsd, pos in zip(
                        names_unsorted, full_ligand_positions_unsorted, rmsds_unsorted
                    )
                }
        ...
        print(
            "Number of RMSDs and positions for the complex graphs: ",
            len(self.full_ligand_positions),
        )
        ...
        self.dataset_names = list(
            set(self.positions_rmsds_dict.keys()) & set(self.complex_graph_dict.keys())
        )
    ...
    def preprocessing(self, original_model_cache):
        ...
        state_dict = torch.load(
            f"{self.original_model_dir}/{self.model_ckpt}",
        ...
        for idx, orig_complex_graph in tqdm(enumerate(loader)):
            ...
            filterHs = (
                torch.not_equal(predictions_list[0]["ligand"].x[:, 0], 0).cpu().numpy()
            )
            ...
            assert (
                len(orig_complex_graph.name) == 1
            )  # I just put this assert here because of the above line where I assumed that the list is always only lenght 1. Just in case it isn't maybe check what the names in there are.
        with open(
            os.path.join(
                self.full_cache_path,
