...
parser.add_argument(
    "--max_lig_size", type=int, default=None, help="Maximum number of heavy atoms"
)
...
parser.add_argument(
    "--max_radius", type=float, default=5.0, help="Radius cutoff for geometric graph"
)
parser.add_argument(
    "--scale_by_sigma",
    action="store_true",
...
parser.add_argument(
    "--ns", type=int, default=16, help="Number of hidden features per node of order 0"
)
parser.add_argument(
    "--nv", type=int, default=4, help="Number of hidden features per node of order >0"
)
...
def train_epoch(model, loader, optimizer, rmsd_prediction):
    ...
    for data in tqdm(loader, total=len(loader)):
        if (
            device.type == "cuda"
            and len(data) % torch.cuda.device_count() == 1
            or device.type == "cpu"
            and data.num_graphs == 1
        ):
        ...
        try:
            ...
            if rmsd_prediction:
                labels = (
                    torch.cat([graph.rmsd for graph in data]).to(device)
                ...
                if isinstance(args.rmsd_classification_cutoff, list):
                    labels = (
                        torch.cat([graph.y_binned for graph in data]).to(device)
                        if isinstance(data, list)
                        else data.y_binned
                    )
                    ...
                    labels = (
                        torch.cat([graph.y for graph in data]).to(device)
                        if isinstance(data, list)
                        else data.y
                    )
...
def test_epoch(model, loader, rmsd_prediction):
    model.eval()
    meter = (
        AverageMeter(["loss"], unpooled_metrics=True)
        if rmsd_prediction
        else AverageMeter(
            ["confidence_loss", "accuracy", "ROC AUC"], unpooled_metrics=True
        )
    )
    all_labels = []
    for data in tqdm(loader, total=len(loader)):
...
def train(args, model, optimizer, scheduler, train_loader, val_loader, run_dir):
    ...
    for epoch in range(args.n_epochs):
        ...
        train_metrics = train_epoch(
            model, train_loader, optimizer, args.rmsd_prediction
        )
        print(
            "Epoch {}: Training loss {:.4f}".format(
                epoch, train_metrics["confidence_loss"]
            )
        )

        val_metrics, baseline_metric = test_epoch(
            model, val_loader, args.rmsd_prediction
        )
        if args.rmsd_prediction:
            print(
                "Epoch {}: Validation loss {:.4f}".format(
                    epoch, val_metrics["confidence_loss"]
                )
            )
        else:
            print(
                "Epoch {}: Validation loss {:.4f}  accuracy {:.4f}".format(
                    epoch, val_metrics["confidence_loss"], val_metrics["accuracy"]
                )
            )

        if args.wandb:
            logs.update(
                {"valinf_" + k: v for k, v in val_metrics.items()}, step=epoch + 1
            )
            logs.update(
                {"train_" + k: v for k, v in train_metrics.items()}, step=epoch + 1
            )
            logs.update(
                {
                    (
        ...
        if (
            ...
            and val_metrics[args.main_metric] < best_val_metric
            or args.main_metric_goal == "max"
            and val_metrics[args.main_metric] > best_val_metric
        ):
        ...
        if (
            args.model_save_frequency > 0
            and (epoch + 1) % args.model_save_frequency == 0
        ):
            torch.save(state_dict, os.path.join(run_dir, f"model_epoch{epoch+1}.pt"))
        if (
            args.best_model_save_frequency > 0
            and (epoch + 1) % args.best_model_save_frequency == 0
        ):
            shutil.copyfile(
...
def construct_loader_confidence(args, device):
    ...
    try:
        ...
        if "The generated ligand positions with cache_id do not exist:" in str(e):
            print(
                "HAPPENING | Encountered the following exception when loading the confidence train dataset:"
            )
            print(str(e))
            print(
    ...
    if exception_flag:
        raise Exception(
            "We encountered the exception during train dataset loading: ", e
        )
...
if __name__ == "__main__":
    ...
    model = get_model(
        score_model_args if args.transfer_weights else args,
        device,
        t_to_sigma=None,
        confidence_mode=True,
    )
    optimizer, scheduler = get_optimizer_and_scheduler(
        args, model, scheduler_mode=args.main_metric_goal
    )

    if args.transfer_weights:
        print(
            "HAPPENING | Transferring weights from original_model_dir to the new model after using original_model_dir's arguments to construct the new model."
        )
        checkpoint = torch.load(
            os.path.join(args.original_model_dir, args.ckpt), map_location=device
        )
        model_state_dict = model.state_dict()
        transfer_weights_dict = {
            k: v for k, v in checkpoint.items() if k in list(model_state_dict.keys())
        }
        model_state_dict.update(
            transfer_weights_dict
        )  # update the layers with the pretrained weights
