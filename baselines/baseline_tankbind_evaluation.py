...
parser.add_argument(
    "--renumbered_atoms_dir",
    type=str,
    default="../TankBind/examples/tankbind_pdb/renumber_atom_index_same_as_smiles",
    help="",
)
parser.add_argument(
    "--results_path",
    type=str,
    default="results/tankbind_top5",
...
for i, name in enumerate(tqdm(names)):
    ...
    assert os.path.exists(
        os.path.join(args.results_path, name, f"{name}_tankbind_0.sdf")
    )
    ligand_pos = []
    for i in range(args.num_predictions):
        if not os.path.exists(
            os.path.join(args.results_path, name, f"{name}_tankbind_{i}.sdf")
        ):
            break
        mol_pred, _ = read_sdf_or_mol2(
            os.path.join(args.results_path, name, f"{name}_tankbind_{i}.sdf"), None
        )
        sm = Chem.MolToSmiles(mol_pred)
        m_order = list(
            mol_pred.GetPropsAsDict(includePrivate=True, includeComputed=True)[
                "_smilesAtomOutputOrder"
            ]
        )
    ...
    try:
        ...
        rmsd = np.array(
            get_symmetry_rmsd(mol, orig_ligand_pos, [l for l in ligand_pos], mol_pred)
        )
    ...
    unsym_rmsds_list.append(
        np.lib.pad(
            unsym_rmsd, (0, 10 - len(unsym_rmsd)), "constant", constant_values=(0)
        )
    )
    rmsds_list.append(
        np.lib.pad(rmsd, (0, 10 - len(rmsd)), "constant", constant_values=(0))
    )
    ...
    centroid_distances_list.append(
        np.lib.pad(
            centroid_distance, (0, 10 - len(rmsd)), "constant", constant_values=(0)
        )
    )
    ...
    ligand_pos_padded = np.lib.pad(
        ligand_pos,
        ((0, 10 - len(ligand_pos)), (0, 0), (0, 0)),
        "constant",
        constant_values=(np.inf),
    )
    ligand_pos_padded_zero = np.lib.pad(
        ligand_pos,
        ((0, 10 - len(ligand_pos)), (0, 0), (0, 0)),
        "constant",
        constant_values=0,
    ...
    cross_distances = np.linalg.norm(
        receptor_pos[:, :, None, :] - ligand_pos_padded[:, None, :, :], axis=-1
    )
    self_distances = np.linalg.norm(
        ligand_pos_padded_zero[:, :, None, :] - ligand_pos_padded_zero[:, None, :, :],
        axis=-1,
    )
    ...
    individual_made_prediction = np.lib.pad(
        np.ones(num_pockets), (0, 10 - len(rmsd)), "constant", constant_values=(0)
    )
...
for overlap in ["", "no_overlap_"]:
    ...
    np.save(
        os.path.join(args.results_path, f"{overlap}names.npy"),
        np.array(successful_names),
    )
    np.save(
        os.path.join(args.results_path, f"{overlap}centroid_distances.npy"),
        centroid_distances,
    )
    np.save(
    ...
    performance_metrics.update(
        {
            f"{overlap}self_intersect_fraction": (
                100
                * (min_self_distances[:, 0] < 0.4).sum()
                / len(min_self_distances[:, 0])
            ),
            f"{overlap}steric_clash_fraction": (
                100
                * (min_cross_distances[:, 0] < 0.4).sum()
                / len(min_cross_distances[:, 0])
            ),
            f"{overlap}mean_rmsd": rmsds[:, 0].mean(),
            f"{overlap}unsym_rmsds_below_2": (
                100 * (unsym_rmsds[:, 0] < 2).sum() / len(unsym_rmsds[:, 0])
            ),
    ...
    top5_centroid_distances = centroid_distances[
        np.arange(rmsds.shape[0])[:, None], np.argsort(inf_rmsds[:, :5], axis=1)
    ][:, 0]
    ...
    top10_centroid_distances = centroid_distances[
        np.arange(rmsds.shape[0])[:, None], np.argsort(inf_rmsds[:, :10], axis=1)
    ][:, 0]
