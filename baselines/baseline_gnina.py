...
parser.add_argument(
    "--file_suffix",
    type=str,
    default="_baseline_ligand",
    help="Path to folder with trained model and hyperparameters",
)
parser.add_argument(
    "--results_path", type=str, default="results/gnina_predictions", help=""
)
parser.add_argument(
    "--complex_names_path", type=str, default="data/splits/timesplit_test", help=""
)
parser.add_argument(
    "--seed_molecules_path",
    type=str,
    default=None,
    help="Use the molecules at seed molecule path as initialization and only search around them",
)
parser.add_argument(
    "--seed_molecule_filename",
    type=str,
    default="equibind_corrected.sdf",
    help="Use the molecules at seed molecule path as initialization and only search around them",
)
...
parser.add_argument(
    "--prank_path", type=str, default="/Users/hstark/projects/p2rank_2.3/prank", help=""
)
...
if args.use_p2rank_pocket and not args.skip_p2rank:
    ...
    with open(f"{p2rank_cache_path}/pdb_list_p2rank.txt", "w") as out:
        for name in names:
            shutil.copy(
                os.path.join(args.data_dir, name, f"{name}_protein_processed.pdb"),
                f"{pdb_files_cache}/{name}_protein_processed.pdb",
            )
...
for i, name in enumerate(names):
    ...
    prediction_output_name = os.path.join(
        args.results_path, name, f"{name}{args.file_suffix}.pdb"
    )
    log_path = os.path.join(args.results_path, name, f"{name}{args.file_suffix}.log")
    if args.seed_molecules_path is not None:
        seed_mol_path = os.path.join(
            args.seed_molecules_path, name, f"{args.seed_molecule_filename}"
        )
    if args.skip_existing and os.path.exists(prediction_output_name):
        continue
    ...
    if args.pocket_mode:
        ...
        if np.any(label):
            ...
            print(
                "No pocket residue below minimum distance ",
                args.pocket_cutoff,
                "taking closest at",
                np.min(d),
            )
    ...
    if id == -1:
        print(
            "rdkit pos could not be generated without using random pos. using random pos now."
        )
    ...
    if args.use_p2rank_pocket:
        df = pd.read_csv(
            f"{p2rank_cache_path}/p2rank_output/{name}_protein_processed.pdb_predictions.csv"
        )
