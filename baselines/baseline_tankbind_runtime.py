...
def parallel_save_prediction(arguments):
    ...
    for idx, line in chosen.iterrows():
        ...
        pred_dist_info = get_info_pred_distance(
            coords,
            y_pred,
            protein_nodes_xyz,
        ...
        new_coords = (
            pred_dist_info.sort_values("loss")["coords"].iloc[0].astype(np.double)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_dir",
        type=str,
        default="/Users/hstark/projects/ligbind/data/PDBBind_processed",
        help="",
    )
    parser.add_argument(
        "--split_path",
        type=str,
        default="/Users/hstark/projects/ligbind/data/splits/timesplit_test",
        help="",
    )
    parser.add_argument(
        "--prank_path",
        type=str,
        default="/Users/hstark/projects/p2rank_2.3/prank",
        help="",
    )
    parser.add_argument(
        "--results_path", type=str, default="results/tankbind_results", help=""
    )
    ...
    parser.add_argument(
        "--skip_multiple_pocket_outputs", action="store_true", default=False, help=""
    )
    ...
    if not args.skip_p2rank:
        for name in names:
            if args.skip_existing and os.path.exists(
                f"{args.results_path}/{name}/{name}_tankbind_1.sdf"
            ):
                continue
    ...
    for name in tqdm(names):
        ...
        clean_res_list, ligand_list = save_cleaned_protein(c, cleaned_protein_path)
        lig, _ = read_mol(
            f"{args.data_dir}/{name}/{name}_ligand.sdf",
            f"{args.data_dir}/{name}/{name}_ligand.mol2",
        )
        ...
        mol = Chem.MolFromMolFile(rdkit_mol_path)
        compound_dict[name + f"_{name}" + "_rdkit"] = (
            extract_torchdrug_feature_from_mol(mol, has_LAS_mask=True)
        )
        ...
        for compound_name in list(compound_dict.keys()):
            ...
            com = ",".join(
                [str(a.round(3)) for a in protein_dict[name][0].mean(axis=0).numpy()]
            )
        ...
        info = pd.DataFrame(
            info, columns=["protein_name", "compound_name", "pocket_name", "pocket_com"]
        )
        ...
        if not args.skip_multiple_pocket_outputs:
            for idx, (dataframe_idx, line) in enumerate(
                copy.deepcopy(output_info_sorted).iterrows()
            ):
                ...
                n_protein = protein_nodes_xyz.shape[0]
                y_pred = (
                    y_pred_list[dataframe_idx].reshape(n_protein, n_compound).to("cpu")
                )
                y = (
                    dataset[dataframe_idx]
                    .dis_map.reshape(n_protein, n_compound)
                    .to("cpu")
                )
                compound_pair_dis_constraint = torch.cdist(coords, coords)
                mol = Chem.MolFromMolFile(rdkit_mol_path)
                LAS_distance_constraint_mask = get_LAS_distance_constraint_mask(
                    mol
                ).bool()
                pred_dist_info = get_info_pred_distance(
                    coords,
                ...
                new_coords = (
                    pred_dist_info.sort_values("loss")["coords"]
                    .iloc[0]
                    .astype(np.double)
                )
                write_with_new_coords(mol, new_coords, toFile)
                if idx < 10:
                    top_10_generation_time.append(
                        time.time() - single_top10_generation_time
                    )
                if idx == 0:
                    top_1_generation_time.append(
                        time.time() - single_top10_generation_time
                    )
        ...
        chosen = output_info_chosen.loc[
            output_info_chosen.groupby(["protein_name", "compound_name"], sort=False)[
                "affinity"
            ].agg("idxmax")
        ].reset_index()

        list_to_parallelize.append(
            (dataset, y_pred_list, chosen, rdkit_mol_path, result_folder, name)
        )
    ...
    if args.num_workers > 1:
        ...
        p.__enter__()
    with tqdm(
        total=len(list_to_parallelize),
        desc=f"running optimization {i}/{len(list_to_parallelize)}",
    ) as pbar:
    ...
    with open(os.path.join(args.results_path, "tankbind_log.log"), "w") as file:
        ...
        file.write(
            f"total_time: {forward_pass_time + times_preprocess + times_inference + top_10_generation_time + top_1_generation_time + p2_rank_time}"
        )
