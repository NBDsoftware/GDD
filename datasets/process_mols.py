...
def get_lig_graph(mol, complex_graph):
    ...
    complex_graph["ligand"].orig_pos = lig_coords
...
def get_calpha_graph(
    ...
    for i in range(num_residues):
        ...
        mean_vec_ratio_norm = (
            np.linalg.norm(mean_vec, axis=1) / denominator
        )  # (sigma_num,)
    ...
    side_chain_vecs = torch.from_numpy(
        np.concatenate(
            [np.expand_dims(n_rel_pos, axis=1), np.expand_dims(c_rel_pos, axis=1)],
            axis=1,
        )
    )

    complex_graph["receptor"].x = (
        torch.cat([node_feat, torch.tensor(lm_embeddings)], axis=1)
...
def get_fullrec_graph(
    ...
    # we save the max distance as approximation of the diameter of a sphere that
    # inscribes the receptor. We use this to approximate the maximum translational
    # distance possible between two points in this sphere
    complex_graph["receptor"].diameter = np.max(distances)
    src_list = []
    dst_list = []
    mean_norm_list = []
    for i in range(num_residues):
    ...
    src_c_alpha_idx = np.concatenate(
        [np.asarray([i] * len(l)) for i, l in enumerate(rec_coords)]
    )
    ...
    atoms_edge_index = radius_graph(
        atom_coords,
        atom_cutoff,
        max_num_neighbors=atom_max_neighbors if atom_max_neighbors else 1000,
    )
    atom_res_edge_index = torch.from_numpy(
        np.asarray([np.arange(len(atom_feat)), src_c_alpha_idx])
    ).long()
    ...
    complex_graph["atom", "atom_rec_contact", "receptor"].edge_index = (
        atom_res_edge_index
    )
...
def write_mol_with_coords(mol, new_coords, path, properties: dict = None):
