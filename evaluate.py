...
parser.add_argument(
    ...
    type=int,
    default=None,
    help="if this is a number instead of none, the max number of cpus used by torch will be set to this.",
)
parser.add_argument("--run_name", type=str, default="test", help="")
parser.add_argument("--project", type=str, default="ligbind_inf", help="")
parser.add_argument(
    "--out_dir", type=str, default=None, help="Where to save results to"
)
parser.add_argument(
    "--batch_size", type=int, default=10, help="Number of poses to sample in parallel"
)
parser.add_argument(
    "--cache_path",
    type=str,
    default="data/cacheNew",
...
parser.add_argument(
    ...
    type=str,
    default="data/splits/timesplit_test_no_rec_overlap",
    help="Path text file with the folder names in the test set that have no receptor overlap with the train set",
)
parser.add_argument(
    "--no_model",
    action="store_true",
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Whether to add randomness in diffusion steps",
)
parser.add_argument(
    "--no_final_step_noise",
    action="store_true",
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Whether to run the probability flow ODE",
)
...
parser.add_argument(
    "--limit_complexes", type=int, default=0, help="Limit to the number of complexes"
)
parser.add_argument(
    "--num_workers", type=int, default=1, help="Number of workers for dataset creation"
)
parser.add_argument(
    "--tqdm", action="store_true", default=False, help="Whether to show progress bar"
)
parser.add_argument(
    "--save_visualisation",
    action="store_true",
...
parser.add_argument(
    ...
    type=int,
    default=1,
    help="Number of poses to sample for each complex",
)
...
if args.confidence_model_dir is not None:
    ...
    if not os.path.exists(confidence_args.original_model_dir):
        ...
        confidence_args.original_model_dir = os.path.join(
            *confidence_args.original_model_dir.split("/")[-2:]
        )
...
test_dataset = PDBBind(
    ...
    matching=not score_model_args.no_torsion,
    keep_original=True,
...
if args.confidence_model_dir is not None:
    if not (
        ...
        confidence_test_dataset = PDBBind(
            ...
            matching=not confidence_args.no_torsion,
            keep_original=True,
            ...
            esm_embeddings_path=confidence_args.esm_embeddings_path,
            require_ligand=True,
            num_workers=args.num_workers,
        )
...
if not args.no_model:
    ...
    if args.confidence_model_dir is not None:
        if confidence_args.transfer_weights:
            with open(
                f"{confidence_args.original_model_dir}/model_parameters.yml"
            ) as f:
    ...
    while not success:  # keep trying in case of failure (sometimes stochastic)
        try:
            ...
            if not args.no_model:
                if confidence_model is not None and not (
                    ...
                    confidence_data_list = [
                        copy.deepcopy(
                            confidence_complex_dict[orig_complex_graph.name[0]]
                        )
                        for _ in range(N)
                    ]
                else:
            ...
            ligand_pos = np.asarray(
                [
                    complex_graph["ligand"].pos.cpu().numpy()[filterHs]
            ...
            try:
                ...
                rmsd = get_symmetry_rmsd(
                    mol, orig_ligand_pos[0], [l for l in ligand_pos]
                )
            except Exception as e:
                print("Using non corrected RMSD because of the error", e)
                rmsd = np.sqrt(
                    ((ligand_pos - orig_ligand_pos) ** 2).sum(axis=2).mean(axis=1)
                )
            rmsds_list.append(rmsd)
            centroid_distance = np.linalg.norm(
                ligand_pos.mean(axis=1) - orig_ligand_pos.mean(axis=1), axis=1
            )
            ...
            if confidence is not None:
                ...
                print(
                    ...
                    " rmsd",
                    np.around(rmsd, 1)[re_order],
                    " centroid distance",
                    np.around(centroid_distance, 1)[re_order],
                    " confidences ",
                    np.around(confidence, 4)[re_order],
                )
            ...
            base_cross_distances = np.linalg.norm(
                receptor_pos[:, :, None, :] - orig_ligand_pos[:, None, :, :], axis=-1
            )
            base_min_cross_distances_list.append(
                np.min(base_cross_distances, axis=(1, 2))
            )
            ...
            without_rec_overlap_list.append(
                1 if orig_complex_graph.name[0] in names_no_rec_overlap else 0
            )
...
for overlap in ["", "no_overlap_"]:
    ...
    np.save(
        f"{args.out_dir}/{overlap}base_min_cross_distances.npy",
        base_min_cross_distances,
    )
    ...
    performance_metrics.update(
        {
            ...
            f"{overlap}self_intersect_fraction": (
                100 * (min_self_distances < 0.4).sum() / len(min_self_distances) / N
            ).__round__(2),
        ...
        performance_metrics.update(
            {
                ...
                f"{overlap}top5_rmsds_percentile_25": np.percentile(
                    top5_rmsds, 25
                ).round(2),
                f"{overlap}top5_rmsds_percentile_50": np.percentile(
                    top5_rmsds, 50
                ).round(2),
                f"{overlap}top5_rmsds_percentile_75": np.percentile(
                    top5_rmsds, 75
                ).round(2),
        ...
        performance_metrics.update(
            {
                ...
                f"{overlap}top10_rmsds_percentile_25": np.percentile(
                    top10_rmsds, 25
                ).round(2),
                f"{overlap}top10_rmsds_percentile_50": np.percentile(
                    top10_rmsds, 50
                ).round(2),
                f"{overlap}top10_rmsds_percentile_75": np.percentile(
                    top10_rmsds, 75
                ).round(2),
    ...
    if confidence_model is not None:
        ...
        filtered_rmsds = rmsds[np.arange(rmsds.shape[0])[:, None], confidence_ordering][
            :, 0
        ]
        ...
        filtered_min_self_distances = min_self_distances[
            np.arange(rmsds.shape[0])[:, None], confidence_ordering
        ][:, 0]
        performance_metrics.update(
            {
                f"{overlap}filtered_self_intersect_fraction": (
                    100
                    * (filtered_min_self_distances < 0.4).sum()
                    / len(filtered_min_self_distances)
                ).__round__(2),
                ...
                f"{overlap}filtered_rmsds_percentile_25": np.percentile(
                    filtered_rmsds, 25
                ).round(2),
                f"{overlap}filtered_rmsds_percentile_50": np.percentile(
                    filtered_rmsds, 50
                ).round(2),
                f"{overlap}filtered_rmsds_percentile_75": np.percentile(
                    filtered_rmsds, 75
                ).round(2),
        ...
        if N >= 5:
            ...
            top5_filtered_min_self_distances = min_self_distances[
                np.arange(rmsds.shape[0])[:, None], confidence_ordering
            ][:, :5][
                np.arange(rmsds.shape[0])[:, None],
                np.argsort(
            ...
            performance_metrics.update(
                {
                    ...
                    f"{overlap}top5_filtered_rmsds_percentile_25": np.percentile(
                        top5_filtered_rmsds, 25
                    ).round(2),
                    f"{overlap}top5_filtered_rmsds_percentile_50": np.percentile(
                        top5_filtered_rmsds, 50
                    ).round(2),
                    f"{overlap}top5_filtered_rmsds_percentile_75": np.percentile(
                    ...
                    f"{overlap}top5_filtered_centroid_percentile_25": np.percentile(
                    ...
                    f"{overlap}top5_filtered_centroid_percentile_75": np.percentile(
        ...
        if N >= 10:
            ...
            top10_filtered_min_self_distances = min_self_distances[
                np.arange(rmsds.shape[0])[:, None], confidence_ordering
            ][:, :10][
                np.arange(rmsds.shape[0])[:, None],
                np.argsort(
            ...
            performance_metrics.update(
                {
                    ...
                    f"{overlap}top10_filtered_centroid_percentile_25": np.percentile(
                    ...
                    f"{overlap}top10_filtered_centroid_percentile_75": np.percentile(
...
if args.wandb:
    ...
    if N >= 5:
        ...
        histogram_metrics_list.append(
            ("top5_centroid_distances", top5_centroid_distances)
        )
    if N >= 10:
    ...
    if confidence_model is not None:
        ...
        histogram_metrics_list.append(
            ("filtered_centroid_distance", filtered_centroid_distances)
        )
        if N >= 5:
            histogram_metrics_list.append(("top5_filtered_rmsds", top5_filtered_rmsds))
            histogram_metrics_list.append(
                ("top5_filtered_centroid_distances", top5_filtered_centroid_distances)
            )
        if N >= 10:
            histogram_metrics_list.append(
                ("top10_filtered_rmsds", top10_filtered_rmsds)
            )
            histogram_metrics_list.append(
                ("top10_filtered_centroid_distances", top10_filtered_centroid_distances)
            )
